...
static struct murl__KeyboardKeyMap murl__keyboard_keys[] = {
    ...
    {KEY_BACKSPACE, MU_KEY_BACKSPACE}, {(KeyboardKey)-1, (KeyboardKey)-1},
};
...
void murl_render(mu_Context *ctx) {
  ...
  while (mu_next_command(ctx, &cmd)) {
    ...
    case MU_COMMAND_TEXT: {
      ...
      Vector2 text_position = MURL_VECTOR2_FROM_MU((float)cmd->text.pos);
    ...
    case MU_COMMAND_RECT: {
      Rectangle rect = MURL_RECTANGLE_FROM_MU((float)cmd->rect.rect);
